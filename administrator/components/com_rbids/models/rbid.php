<?php
	...
	class JRbidsAdminModelRbid extends JModel
	{
		...
		public function saveAuction($auct)
		{
			...
			// Check if auction was not closed by admin before save
			$auctionCK = JTable::getInstance('auctions', 'Table');
			$auctionCK->load($auct->id);
			// Send email notification in case the auction is closed by admin
			if (0 == $auctionCK->close_by_admin && 1 == $auct->close_by_admin) {

				$user = JFactory::getUser($auct->userid);
				$auct->SendMails(array($user), 'bid_admin_close_auction');
			}
		...
		public function saveInvites($auct)
		{
			...
			$invites = JModel::getInstance('invites', 'JRbidsAdminModel');
		...
		public function deleteFiles($auction)
		{
			...
			$delete_NDA = JRequest::getVar('delete_NDA', null);
			...
			if ($delete_atachment) {
				$query = "UPDATE `#__rbid_auctions` SET `has_file` = 0, `file_name` = '' WHERE `id` = {$id}";
				$database->setQuery($query);
				$database->query();
			...
			if ($delete_NDA) {
				$query = "UPDATE `#__rbid_auctions` SET `NDA` = 0, `NDA_file` = '' WHERE `id` = {$id}";
				$database->setQuery($query);
				$database->query();

				@unlink(AUCTION_UPLOAD_FOLDER . "{$id}.nda");
			}
		...
		public function uploadFiles(&$auction)
		{
			...
			foreach ($_FILES as $k => $file) {
				...
				if ($k == "picture_0" && (!$auction->picture || $delete_main_picture)) {
		...
		public function validateSave($item)
		{
			...
			if ($cfg->enable_attach) {
				...
				} elseif ($cfg->attach_compulsory && !$item->has_file)
			...
			if ($cfg->nda_option) {
				...
				} elseif ($cfg->nda_compulsory && !$item->NDA)
