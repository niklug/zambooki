<?php
	...
	class UserController extends JController
	{
		...
		public function UserDetails()
		{
                        ...
                        echo "<pre>";print_r($trace);echo"</pre>";exit;
...
//                        $dispatcher->trigger('onContentPrepareData', array('com_users.profile', &$juser));                        
//                        echo "<pre>";print_r($juser);echo "</pre>";
                        //get Profile from com_community and set as default*****************
                        $CUser = CFactory::getUser();
                        $CProfileModel = CFactory::getModel('profile');
                        $CProfile       = $CProfileModel->getEditableProfile($CUser->id, $CUser->getProfileType() );
//                        echo"<pre>";print_r($CProfile);echo"</pre>";
                        $user->phone = ($user->phone != '') ? $user->phone : $CProfile['fields']['Contact Information'][0]['value'];
                        $user->about_me = ($user->about_me != '') ? $user->about_me : $CProfile['fields']['Basic Information'][2]['value'];
                        $user->address = ($user->address != '') ? $user->address : $CProfile['fields']['Contact Information'][2]['value'];
                        $user->city = ($user->city != '') ? $user->city : $CProfile['fields']['Contact Information'][4]['value'];
                        $user->country = ($user->country != '') ? $user->country : strtoupper($CProfile['fields']['Contact Information'][5]['value']);
                        //*******************************************
                        echo $user->country ;
