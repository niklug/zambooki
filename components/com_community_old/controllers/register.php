...
class CommunityRegisterController extends CommunityBaseController
{
	...
	public function register_save()
	{
		...
		$fields       =  $modelProfile->getAllFields($filter);
                ...
                //get profile type
                require_once(JPATH_ROOT.DS.'components'.DS.'com_xipt'.DS.'libraries'.DS.'lib'.DS.'pluginhandler.php');
                $Pluginhandler = new XiptLibPluginhandler;
                $pt = $Pluginhandler->getRegistrationPType();
                
		// If there are no fields or PROFILE IS "HOME OWNER", we do not want to move to the edit profile area.
		if (count($fields) <= 0 || $pt == 6)
		{
	...
	public function ajaxSetMessage($fieldName, $txtLabel = '', $strMessage, $strParam='', $strParam2='')
	{
		...
		$objResponse->addScriptCall('joms.jQuery("#err'.$fieldName.'msg").html("<br/>'.trim($langMsg).'");');
