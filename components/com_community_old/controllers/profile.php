...
class CommunityProfileController extends CommunityBaseController
{
	...
	public function save()
	{
		...
		if(property_exists($jUser, 'groups')){
                        //that's because it doesn't work without it
                        $jU_groups = $jUser->groups;
                        if($jUser->field17 == 4 && !isset($jU_groups[9]) ) $jU_groups[9]=9;
                        if($jUser->field17 == 6 ) {
                            unset($jU_groups[9]);
                            unset($jU_groups['Professional-Contractor']);
                        }
                        foreach($jU_groups as $groupid => $groupname){
                                if($groupid == 'Professional-Contractor') $groupid = 9;
				$jU_groups[$groupid] = $groupid;
			}
                        $jUser->groups = $jU_groups;
		}
		...
		if($editSuccess)
		{
			$session = JFactory::getSession();
                        ...
                        $jUser->getAuthorisedGroups(true);
                        $jUser->getAuthorisedViewLevels(true);
                        $session->set('user', $jUser);
			...
			$appsLib->triggerEvent( 'onUserDetailsUpdate', $userRow );
                        $appsLib->triggerEvent( 'onSetRbidUser', $userRow );
