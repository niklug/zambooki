...
$usera = JRequest::getVar('userid');
$Itemid = JRequest::getVar('Itemid');
$query->select(array('a.*', 'b.name', 'c.title'));
$query->from('#__rbid_rate AS a');
$query->join('INNER', '#__rbid_users AS b ON (a.voter = b.userid)');
$query->join('LEFT', '#__rbid_auctions AS c ON (a.auction_id = c.id)');
$query->where('a.user_rated = '.$usera);
...
<?php if($results){ ?>
var jako = '<div class="RB"> '
+ '<div class="head">Last 10 reviews for auctions</div>'
+ '<div class="resurses">'
+ '<?php foreach($results as $res){ 
echo '<div class="rowi"><a href="index.php?option=com_community&view=profile&userid='.$res['voter'].'&Itemid='.$Itemid.'">'.$res['name'].'</a> rated with <span class="mess">'.$res['rating'].' stars</span> and left message " <span class="mess">'.$res['message'].'</span> " for auction <a href="index.php?option=com_rbids&task=viewbids&id='.$res['auction_id'].'">'.$res['title'].'</a></div>'; } ?> '
+ '</div>'
+ '</div>';
<?php }else{
}?>

jQuery("div.activity-stream-front").after(jako);
});
</script>