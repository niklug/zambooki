...
// hack begin
	...
	public function ajaxLoadContractorsList( $namePrefix, $callback, $cid, $limitstart = 0, $limit = 8  )
	{
		 // pending filter
		$objResponse	= new JAXResponse();
		$filter = JFilterInput::getInstance();
		...
		$handler	= CFactory::getModel($handlerName);
		$friends = $handler->getContractorListByName($namePrefix,$my->id,$cid,$limitstart,$limit);
		...
		$tmpl			= new CTemplate();
		$tmpl->set( 'friends'	, $friends );
		$tmpl->set( 'selected'	, array($handler->getContractorSelected($cid)));
		$tmplName = 'ajax.friend.list.contractors';
		$html = $tmpl->fetch( $tmplName );
		...
		if($handler->total > $loadedFriend){
			...
			$moreCount = $handler->total - $loadedFriend;
			//load more option
			$loadMore = '<a onClick="joms.friends.loadMoreContractor(\''. $callback.'\',\''. $cid.'\',\''.$limitstart.'\',\''.$limit.'\');" href="javascript:void(0)">'.JText::_('COM_COMMUNITY_INVITE_LOAD_MORE').'('.$moreCount.') </a>';
		} else {
			//nothing to load
			$loadMore = '';
		}
