...
class CommunityViewPhotos extends CommunityView
{
	...
	public function albumall()
	{
		...
		$albumId	= $jinput->get->get('albumid' , 1, 'INT');
		...
		$document->setTitle( 'Project Gallery *(Click Photo to Request Bids on a Similar Project)' );
		$this->setTitle(  'Project Gallery *(Click Photo to Request Bids on a Similar Project)' );
		...
		{
			$photo = $photos[$i];
			$photo->link = $handler->getPhotoURIall($photo->id , $photo->albumid, $this->getAlbumCreator($photo->albumid));
		}
        ...
        /*
         * find id of album creator on basis of album id
         */
        public function getAlbumCreator($albumId) {
            $db = &JFactory::getDBO();
            $query = "SELECT creator FROM #__community_photos_albums WHERE id='$albumId'";
            $db->setQuery( $query );
            $albumCreator =  $db->loadResult();
            return $albumCreator;
        }
	/**
	 * Display the photo thumbnails from an album
...
//		$friends	= $model->getFriends( $this->my->id , '' , false );
	...
	public function getPhotoURI( $photoId , $albumId )
	{
        ...
        public function getPhotoURIall( $photoId , $albumId, $creator )
	{
		return CRoute::_('index.php?option=com_community&view=photos&task=photo&userid=' . $creator . '&albumid=' . $albumId . '&photoid=' . $photoId);
	}
