...
implements CLimitsInterface
{
	...
	public function getPhotos( $id , $limit = null , $limitstart = null, $includeUnpublished = false )
	{
		...
		if(!$includeUnpublished){
        	$includeUnpublished = ' AND `published`=' . $db->Quote( 1 );
                $includeUnpublishedwhere = ' WHERE `published`=' . $db->Quote( 1 );
		}else{
        	$includeUnpublished = ''; //include
                $includeUnpublishedwhere = '';
		}
	...
	public function getAllAlbums( $userId = 0, $limit = 0 )
	{
		...
		$subQuery	= 'SELECT COUNT( DISTINCT(s.id) ) '
					. 'FROM ' . $db->quoteName( '#__community_photos_albums' ) . ' AS s '
					. 'RIGHT JOIN ' . $db->quoteName( '#__community_groups_members' ) . ' AS t '
		...
		$query	= 'SELECT x.*, '
				...
				. '	  true '
				. '	WHEN 20 THEN '
				. '	  (SELECT COUNT(u.id) FROM ' . $db->quoteName( '#__users' ) . ' AS u WHERE u.block=0 AND u.id=' . $db->Quote( $userId ) . ') '
				. '	WHEN 30 THEN '
				. '	  IF( a.creator=' . $db->Quote( $userId ) . ' or ' . $isAdmin . ', true, (SELECT COUNT(v.connection_id) FROM ' . $db->quoteName('#__community_connection') . ' AS v WHERE v.connect_from=a.creator AND v.connect_to=' . $db->Quote( $userId ) . ' AND v.status=1) ) '
				. '	WHEN 40 THEN '
				...
				. 'ON a.photoid=c.id '
				. 'LEFT JOIN ' . $db->quoteName( '#__community_groups' ) . ' AS d '
				. 'ON a.groupid=d.id '
				. 'GROUP BY a.id '
...
// hack begin
    ...
    public function getContractorSelected($cid) {
		$db	= $this->getDBO();

        $sql = "SELECT t.userid"
            ."\nFROM #__community_photos_tag AS t"
            ."\nWHERE t.photoid=".(int)$cid
            ;
	...
	public function getContractorListByName($namePrefix ,$userid, $cid, $limitstart = 0, $limit = 8){
		$db	= $this->getDBO();
		$my = CFactory::getUser();
		$andName = '';
		$config = CFactory::getConfig();
		...
		if($album->groupid){
    		...
    		if(!empty($namePrefix)){
                $andName = "\n\tAND `$nameField` LIKE ".$db->Quote('%'.$namePrefix.'%');
            }

            $sql = "SELECT u.id"
                ."\nFROM #__users AS u"
                ."\nLEFT JOIN #__rbid_auctions AS a ON a.winner_id=u.id"
                ."\nWHERE a.userid=".(int)$my->id
                .$andName
                ."\nORDER BY u.`$nameField`"
                ."\nLIMIT $limitstart, $limit";
                ;
            ...
            $sql = "SELECT COUNT(u.id)"
                ."\nFROM #__users AS u"
                ."\nLEFT JOIN #__rbids_auctions AS a ON a.winner_id=u.id"
                ."\nWHERE a.userid=".(int)$my->id
                .$andName
                ;
        ...
        $selected = $this->getContractorSelected($cid);

		if ($selected && !in_array($selected, $contractors)) {
            array_unshift($contractors, $selected);
