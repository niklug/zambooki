...
class CommunityModelProfile extends JCCModel
{
	...
	public function saveProfile($userId, $fields)
	{
		...
		foreach($fields as $id => $value)
		{
			...
			$table->store();
                        $db->setQuery("SELECT fieldcode FROM #__community_fields WHERE id=".$id." ");
                        $fieldCode = $db->loadResult();
                        $FieldsForRbids[strtolower(str_replace("FIELD_", "", $fieldCode))] = $table->value;
		}

                $db->setQuery(
                    'DELETE FROM #__rbid_users WHERE userid = '. $user->id ." "
                );
		...
		$longitude	= COMMUNITY_LOCATION_NULL;
                ...
                $address .=($FieldsForRbids['state'] != '') ? $FieldsForRbids['state']." state ": " ";
                $address .=($FieldsForRbids['city'] != '') ? $FieldsForRbids['city']." city ": " ";
                if($FieldsForRbids['state'] == '' && $FieldsForRbids['city'] == '') $address = $FieldsForRbids['address'];
//                $address = rawurlencode($address); //it performe in CMapping::getAddressData()
		...
		if($data){
			if($data->status == 'OK')
			{
                ...
                $db->setQuery("
                    INSERT INTO #__rbid_users SET
                        userid='{$user->id}',
                        name='{$user->name}',
                        address='{$FieldsForRbids['address']}',    
                        city='{$FieldsForRbids['city']}',    
                        country='".strtoupper($FieldsForRbids['country'])."',    
                        phone='{$FieldsForRbids['mobile']}',    
                        about_me='{$FieldsForRbids['aboutme']}',
                        googleMaps_x='{$latitude}',
                        googleMaps_y='{$longitude}',
                        modified=NOW()    
                ");
                if (!$db->query()){throw new Exception($db->getErrorMsg());}
                return true;
